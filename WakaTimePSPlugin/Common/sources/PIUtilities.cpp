					 	 ...
					 	 size_t *outSize)
{
	...
	*outSize = 0;
	...
	if (inType == StringResource)
	{
		...
		size = LoadStringA(GetDLLInstance(), inRezID, buff, kStr255MaxLen);
		if (size > 0)
		{ // got something
	...
	else
	{
		...
		h2 = FindResourceA(GetDLLInstance(), winID, name);
		if (h2 != NULL)
		{
			size_t size = 0;
...
OSErr GetOSInfo(HandleProcs* /* procs */, OSInfo *osInfo)
{
	...
	GetModuleFileNameA(NULL /* GetDLLInstance() */, filename, kStr255MaxLen);
	versionInfoSize = (int16)GetFileVersionInfoSizeA(filename, &versionInfoHandle);
	...
	*/
	...
	winErr = (short)(GetFileVersionInfoA(filename, 
		                                versionInfoHandle, 
										versionInfoSize, 
										versionInfoData));
	...
	{ // no error.  Continue.
		...
		winErr = (short)(VerQueryValueA(versionInfoData, 
			                           "\\", 
									   (LPVOID*)&p, 
